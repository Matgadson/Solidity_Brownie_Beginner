...
function createErrorMiddleware() {
  return (req, res, next) => {
    ...
    if (typeof req.method !== "string" || !req.method) {
      res.error = rpc_errors_namespaceObject.rpcErrors.invalidRequest({
...
class CommunicationProvider extends baseProvider {
  ...
  handleDisconnect(isRecoverable, errorMessage) {
    if (this.state.isConnected || !this.state.isPermanentlyDisconnected && !isRecoverable) {
      ...
      let error;
      if (isRecoverable) {
        error = new rpc_errors_namespaceObject.EthereumProviderError(1013,
        // Try again later
        errorMessage || messages.errors.disconnected());
        loglevel.debug(error);
      } else {
        error = new rpc_errors_namespaceObject.EthereumProviderError(1011,
        // Internal error
...
class WsEmbed {
  constructor() {
    ...
    defineProperty_default()(this, "walletIframe", void 0);
  ...
  async setupWeb3(providerParams) {
    ...
    communicationProvider.tryWindowHandle = (payload, cb) => {
      ...
      if (!Array.isArray(_payload) && COMMUNICATION_UNSAFE_METHODS.has(_payload.method)) {
        ...
        communicationProvider.handleWindow(windowId, {
          target: "_blank",
          features: (0,base_controllers_namespaceObject.getPopupFeatures)(base_controllers_namespaceObject.FEATURES_PROVIDER_CHANGE_WINDOW) // todo: are these features generic for all
        });
