...
import { createLoggerMiddleware, getPopupFeatures, FEATURES_DEFAULT_POPUP_WINDOW, COMMUNICATION_JRPC_METHODS, COMMUNICATION_NOTIFICATIONS, PROVIDER_JRPC_METHODS, PROVIDER_NOTIFICATIONS, randomId, FEATURES_CONFIRM_WINDOW, FEATURES_PROVIDER_CHANGE_WINDOW } from '@toruslabs/base-controllers';
...
class CommunicationProvider extends BaseProvider {
  constructor(connectionStream, _ref, state) {
    let {
      maxEventListeners = 100,
      jsonRpcStreamName = "provider"
    } = _ref;
...
const COMMUNICATION_UNSAFE_METHODS = new Set([COMMUNICATION_JRPC_METHODS.SET_PROVIDER]);
...
class WsEmbed {
  constructor() {
    ...
    _defineProperty(this, "walletIframe", void 0);
  ...
  async init() {
    ...
    // Iframe code
    this.walletIframe = htmlToElement(`<iframe
        id="walletIframe"
        class="walletIframe"
  ...
  async setupWeb3(providerParams) {
    ...
    inPageProvider.tryWindowHandle = (payload, cb) => {
      const _payload = payload;
      if (!Array.isArray(_payload) && PROVIDER_UNSAFE_METHODS.has(_payload.method)) {
...
export { BUTTON_POSITION, LOGIN_PROVIDER, TorusInPageProvider, WS_EMBED_BUILD_ENV, WsEmbed as default };
